...
class BrainRubyProdTest < Minitest::Test
  def setup
    BrainRuby.configure_with("#{GEM_ROOT}/conf_prod.yml")
  ...
  def test_url_raises_error_if_verifying_ssl
    assert_raises OpenSSL::SSL::SSLError do
      BrainRuby::Requests.url("wdc", @test_url)
    end
  end

  def test_url_success_and_returns_brain_url
    result = BrainRuby::Requests.url("wdc", @test_url, verify: false)
